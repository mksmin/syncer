...
Syncer — личный vibe-coded проект, который автор `mksmin` делает прежде всего для себя. Проект
публикуется «как есть», без гарантий корректной работы, сохранности файлов и совместимости с вашей
конфигурацией Obsidian или Яндекс Диска.

Синхронизация не является резервным копированием. Ошибка, неверная настройка или неожиданное
поведение API могут привести к изменению либо удалению локальных файлов. Автор не несёт
ответственности за потерю данных. Используйте Syncer только на свой страх и риск и обязательно
сделайте независимую резервную копию vault перед первым запуском и обновлениями плагина.
...
`Ручная синхронизация в фоне` запускает sync без подтверждения. То же правило используют sync при
запуске и периодический таймер. Таймер работает, пока Obsidian активен; iOS может заморозить
приложение в фоне. Уведомление о старте автоматического sync можно отключить в настройках. Удаления
при любом фоновом запуске запрещены.
...
её можно кнопкой в окне или командой `Остановить текущую синхронизацию`.

В Command Palette доступны: `Ручная синхронизация в фоне`, `Создать новый план синхронизации`,
`Показать статус синхронизации`, `Остановить текущую синхронизацию`, `Открыть настройки` и
`Проверить подключение к Яндекс Диску`. Obsidian автоматически добавляет к названиям префикс
`Syncer:`.
...
`Ручная синхронизация в фоне`: операция пойдёт по правилу настроек. Startup sync запускается после
layout-ready delay; периодический sync — по выбранному интервалу. Оба никогда не удаляют файлы.
...
[THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md). Выпуск: [docs/releasing.md](docs/releasing.md).
